...
export class ReportExportGiaoNhanStrategy extends AbstractReportExportStrategy<
...
> {
  ...
  async prepareTableConfig(): Promise<TableConfig<Sample>> {
    ...
    return {
      name: `DS Giao nhan mau (${format(parseISO(this.options.fromDate), DATEONLY_FORMAT.replaceAll('/', '_'))} - ${format(parseISO(this.options.toDate), DATEONLY_FORMAT.replaceAll('/', '_'))}).xlsx`,
      items,
