...
export abstract class BaseMongoService<Entity> {
  ...
  public async exists(filter: FilterQuery<Entity>): Promise<boolean> {
    return Boolean((await this.model.exists(filter))?._id)
  }
  ...
  public async aggregate(pipelines: Array<PipelineStage>): Promise<Array<any>> {
    return await this.model.aggregate(pipelines).allowDiskUse(true).exec()
  }
