import { ID_TEST_THINPREP } from '../../../../../../libs/levansy-common/src'
...
@Injectable()
export class ThinprepService extends BaseExportService<ExportPapsmearRequestDto> {
  constructor(
    private sampleService: SampleService,
    private testElementService: TestElementService,
    private userService: UserService,
  ) {
    super(ThinprepService.name)
  }

  async prepareAOA(body: ExportPapsmearRequestDto) {
    const [paraElement, chanDoanElement, ketLuanElement] =
      await this.testElementService.getElementsForTestReport([ID_TEST_THINPREP])
    ...
    for (const sample of samples) {
      ...
      const testResult = sample.results.find(
        ({ testId }) => testId === ID_TEST_THINPREP,
      )
