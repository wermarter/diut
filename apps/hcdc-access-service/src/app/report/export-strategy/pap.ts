...
export class ReportExportPapStrategy extends AbstractReportExportStrategy<
...
> {
  ...
  async fetchData() {
    ...
    const testIds = reportConfig[ReportType.Pap]?.testIds
    if (testIds === undefined || testIds.length !== 1) {
      throw new EEntityNotFound(
        `branchId=${this.options.branchId} missing reportConfig for ${ReportType.Pap}`,
      )
    }
  ...
  async prepareTableConfig(): Promise<TableConfig<Sample>> {
    ...
    return {
      name: `7 So Pap Smear (${format(parseISO(this.options.fromDate), DATEONLY_FORMAT.replaceAll('/', '_'))} - ${format(parseISO(this.options.toDate), DATEONLY_FORMAT.replaceAll('/', '_'))}).xlsx`,
      items,
